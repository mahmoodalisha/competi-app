...
export const useLivePrices = (marketIds: string[]) => {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(process.env.NEXT_PUBLIC_CLOB_WS_URL || 'wss://ws-subscriptions-clob.polymarket.com/ws');
      ...
      if (data.type === 'orderbook_update') {
        setPrices(prev => ({
          ...
          [data.market_id]: {
            ...
            bestAsk: data.asks && data.asks.length > 0 ? parseFloat(data.asks[0].price) : 0
          }
        }));
      }
    };
